import { IAccessNonceStore, IAuthenticationKeyStore, IAuthenticationNonceStore, IDigester, IPassphraseAuthenticationKeyStore, IPassphraseRegistrationTokenStore, IRefreshKeyStore, IRefreshNonceStore, IRegistrationTokenStore, ISalter, ISigningKey, IVerificationKey, IVerifier } from "../interfaces";
import { AccessRequest, AccessToken, BeginAuthenticationRequest, BeginAuthenticationResponse, BeginPassphraseAuthenticationRequest, BeginPassphraseAuthenticationResponse, CompleteAuthenticationRequest, CompleteAuthenticationResponse, CompletePassphraseAuthenticationRequest, CompletePassphraseAuthenticationResponse, PassphraseRegistrationMaterials, RefreshAccessTokenRequest, RefreshAccessTokenResponse, RegisterAuthenticationKeyRequest, RegisterAuthenticationKeyResponse, RegisterPassphraseAuthenticationKeyRequest, RegisterPassphraseAuthenticationKeyResponse, RegistrationMaterials, RotateAuthenticationKeyRequest, RotateAuthenticationKeyResponse } from "../messages";
...
export class BetterAuthServer {
    ...
    registerAuthenticationKey(message: string): string {
        ...
        if (!request.verify(this.crypto.verification.key, request.payload.authentication.publicKeys.current)) {
    ...
    registerPassphraseAuthenticationKey(message: string): string {
        ...
        if (!request.verify(this.crypto.verification.passphrase, request.payload.passphraseAuthentication.publicKey)) {
            throw 'invalid signature';
        }

        const token = request.payload.registration.token;
        const [accountId, salt, parameters] = this.stores.registrationToken.passphrase.validate(token);
        const passphraseKeyDigest = this.crypto.digest.sum(request.payload.passphraseAuthentication.publicKey);

        this.stores.key.passphrase.register(
            accountId,
            passphraseKeyDigest,
            salt,
            parameters
    ...
    beginAuthentication(message: string): string {
        ...
        const nonce = this.stores.nonce.authentication.generate(request.payload.identification.accountId);
    ...
    completeAuthentication(message: string): string {
        ...
        const accountId = this.stores.nonce.authentication.validate(request.payload.authentication.nonce);
    ...
    completePassphraseAuthentication(message: string): string {
        ...
        const accountId = this.stores.nonce.authentication.validate(request.payload.passphraseAuthentication.nonce);
        const publicKeyDigest = this.crypto.digest.sum(request.payload.passphraseAuthentication.publicKey);
